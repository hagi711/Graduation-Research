...
# --- キャリブレーション読み込み ---
with open('calibration_data_circle.pkl', 'rb') as f:
    calib = pickle.load(f)
...
objp = np.zeros((circlegridSize[0] * circlegridSize[1], 3), np.float32)
objp[:, :2] = np.mgrid[0:circlegridSize[0], 0:circlegridSize[1]].T.reshape(-1, 2)
...
for img0_path, img2_path in zip(images0, images2):
    ...
    ret0, corners0 = cv.findCirclesGrid(gray0, circlegridSize, None)
    ret2, corners2 = cv.findCirclesGrid(gray2, circlegridSize, None)
    ...
    if ret0 and ret2:
        ...
        '''
        cv.drawChessboardCorners(img0, circlegridSize, corners0, ret0)
        cv.imshow('img0', img0)

        cv.drawChessboardCorners(img2, circlegridSize, corners2, ret2)
        cv.imshow('img2', img2)
        cv.waitKey(500)
        '''
...
# 保存ファイル名
filename = 'stereo_calibration_data_circle.pkl'
