...
objp1 = np.zeros((circlegridSize[0] * circlegridSize[1], 3), np.float32)
objp1[:, :2] = np.mgrid[0:circlegridSize[0], 0:circlegridSize[1]].T.reshape(-1, 2)

# Size of chessboard squares in mm
size_of_circlegrid_mm = 18
objp1 = objp1 * size_of_circlegrid_mm
objp2 = objp1
...
for image in images1:
    ...
    ret, corners = cv.findCirclesGrid(gray, circlegridSize, None)
    ...
    if ret:
        ...
        cv.drawChessboardCorners(img, circlegridSize, corners, ret)
        cv.imshow('img', img)
        cv.waitKey(500)
        '''
...
'''
# 保存ファイル名
filename = 'calibration_data_circle.pkl'
